...
public class ProxyClientSession extends ClientSession {
    ...
    public ProxyClientSession(Channel channel, String connUuid, ServerType serverType, InetSocketAddress address, String username, String password) {
        ...
        if (getProxyClientRealmSession() != null) {
            getProxyClientRealmSession().sendMessage(PacketOuterClass.Opcode.CMSG_LOGIN_REQUEST, Shared.LoginRequest.newBuilder().setUsername(username).setPassword(password).buildPartial());
        }
    }
    ...
    public void disconnectToRealm(String accountName, String password) {
        if (connect(ServerType.REALM, ProxyRouting.getRealmAddress())) {
            ...
            if (getProxyClientRealmSession() != null) {
                NetworkBootstrap.sendPacket(getProxyClientRealmSession().getChannel(), PacketOuterClass.Opcode.CMSG_LOGIN_REQUEST, Shared.LoginRequest.newBuilder().setUsername(accountName).setPassword(password).build());
            }
            return;
        } else {
     ...
     */
    public void transferToWorld(int characterId, String characterName, String connectionUuid, int accountId, String accountName, String password) {
        if (getProxyClientWorldSession() != null) {
            NetworkBootstrap.sendPacket(getProxyClientWorldSession().getChannel(), PacketOuterClass.Opcode.P_WorldCharacterInformation, Proxy.WorldCharacterInformation.newBuilder().setCuuid(connectionUuid).setToken(CLIENT_TOKEN).
                    setCharacterId(characterId).setCharacterName(characterName).setAccountId(accountId).setAccountName(accountName).setPassword(password).build());
    ...
    public void messageReceived(Object message) {
        if (message instanceof PacketOuterClass.Packet) {
            ...
            if (type != null) {
                ...
                if (!opcode.equals(PacketOuterClass.Opcode.CMSG_TRANSFORM_UPDATE))
                    SystemLogger.sendSystemMessage("Routing Client Message to " + type);
